    ...
    return (
        <div className="w-[616px] bg-white rounded-[16px] shadow-sm border border-[#e6e6e6] pt-[40px] px-[80px] pb-[40px] mx-auto my-8 relative flex flex-col items-center" >
            ...
            <div className="flex flex-col gap-[24px] w-full">
                ...
                {/* Gender */}
                <div className="flex flex-col gap-[8px] items-start w-full">
                    <label className="text-[14px] text-[#191919] text-left">性別</label>
                    <div className="flex gap-8">
                        <label className="flex items-center gap-2 cursor-pointer">
                            <div
                                className={`w-5 h-5 border-2 rounded-sm flex items-center justify-center ${formData.gender === 'male' ? 'border-[#009bcd] bg-white' : 'border-[#b3b3b3]'}`}
                                onClick={() => handleGenderSelect('male')}
                            >
                                {formData.gender === 'male' && <div className="w-3 h-3 bg-[#009bcd]" />}
                            </div>
                            <span className="text-[16px] text-[#191919]">男</span>
                        </label>
                        <label className="flex items-center gap-2 cursor-pointer">
                            <div
                                className={`w-5 h-5 border-2 rounded-sm flex items-center justify-center ${formData.gender === 'female' ? 'border-[#009bcd] bg-white' : 'border-[#b3b3b3]'}`}
                                onClick={() => handleGenderSelect('female')}
                            >
                                {formData.gender === 'female' && <div className="w-3 h-3 bg-[#009bcd]" />}
                            </div>
                            <span className="text-[16px] text-[#191919]">女</span>
                        </label>
                        <label className="flex items-center gap-2 cursor-pointer">
                            <div
                                className={`w-5 h-5 border-2 rounded-sm flex items-center justify-center ${formData.gender === 'other' ? 'border-[#009bcd] bg-white' : 'border-[#b3b3b3]'}`}
                                onClick={() => handleGenderSelect('other')}
                            >
                                {formData.gender === 'other' && <div className="w-3 h-3 bg-[#009bcd]" />}
                            </div>
                            <span className="text-[16px] text-[#191919]">其他</span>
                        </label>
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-[8px] items-start w-full">
                    ...
                    <div className="flex justify-between items-center border-b border-[#009bcd] pb-2 w-[454px]">
                        <input
                            ...
                            readOnly
                            disabled
                            className="w-full text-[16px] text-[#191919] focus:outline-none bg-transparent cursor-not-allowed"
                            placeholder=""
                        />
                        <span className="text-[#009bcd] text-[12px] whitespace-nowrap">已驗證</span>
                    </div>
                </div>
