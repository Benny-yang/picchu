...
const ActivityApplicationModal: React.FC<ActivityApplicationModalProps> = ({ isOpen, onClose, onSubmit, activityTitle, activityTime }) => {
    ...
    return (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-xl w-full max-w-lg p-8 relative shadow-2xl">
                {/* Close Button */}
                {/* Design doesn't explicitly show a close X, but usually needed for UX. Figma might imply clicking outside or a Cancel button. 
                    I'll add a subtle X or just rely on outside click (parent handles outside click usually). 
                    Let's add a clean X for safety. */}
                <button
                    onClick={onClose}
                ...
                {/* Title */}
                <h2 className="text-xl font-bold text-center mb-8 text-[#191919]">入團申請</h2>

                {/* Info Fields */}
                <div className="space-y-4 mb-8">
                    <div className="flex text-lg text-[#666666]">
                        <span className="w-16 flex-shrink-0">時間:</span>
                        <span className="text-[#191919] tracking-wider">{activityTime}</span>
                    </div>
                    <div className="flex text-lg text-[#666666]">
                        <span className="w-16 flex-shrink-0">團名:</span>
                    ...
                    <div className="flex items-center gap-6 text-lg text-[#666666] mt-2">
                        <div>
                            已參加: <span className="text-[#191919] font-bold">7</span>
                        </div>
                        <div>
                            人數上限: <span className="text-[#191919] font-bold">15</span>
                        </div>
                        <div>
                            審核中: <span className="text-red-500 font-bold">22</span>
                        </div>
                    </div>
                </div>
