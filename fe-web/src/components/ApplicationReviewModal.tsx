...
const ApplicationReviewModal: React.FC<ApplicationReviewModalProps> = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
            ...
            >
                {/* Left Column: List */}
                <div className="w-[400px] border-r border-[#e6e6e6] flex flex-col bg-white">
                    <div className="p-4 border-b border-[#e6e6e6] text-center">
                        <h3 className="font-bold text-lg text-[#191919]">審核名單</h3>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto">
                        {APPLICANTS.map((applicant) => (
                            <div
                                ...
                                className={`p-4 flex items-center gap-3 cursor-pointer hover:bg-gray-50 transition-colors ${selectedApplicantId === applicant.id ? 'bg-blue-50/50' : ''}`}
                            >
                                ...
                                <div>
                                    ...
                                    {applicant.status === 'pending' && (
                                        <span className="px-3 py-1 border border-red-500 text-red-500 rounded-full text-xs">待審核</span>
                                    )}
                                </div>
                            </div>
                        ))}
                ...
                <div className="flex-1 flex flex-col bg-white relative">
                    ...
                    <div className="p-4 border-b border-[#e6e6e6] text-center">
                        <h3 className="font-bold text-lg text-[#191919]">入團申請</h3>
                    </div>
                    ...
                    <div className="flex-1 p-8 flex flex-col items-center overflow-y-auto">
                        ...
                        <div className="flex gap-4 w-full max-w-sm mt-auto pb-8">
                            <button className="flex-1 py-3 px-6 rounded-full border border-[#009bcd] text-[#009bcd] font-bold hover:bg-blue-50 transition-colors">
                                不同意
                            </button>
                            <button className="flex-1 py-3 px-6 rounded-full bg-[#009bcd] text-white font-bold hover:bg-[#0089b6] transition-colors shadow-md hover:shadow-lg">
                                同意
                            </button>
                        </div>
                    </div>
