...
const LoginPopup: React.FC<LoginPopupProps> = ({ onLogin, onRegister, onClose, onGuest }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center font-sans">
            ...
            {/* Modal Card - Responsive and Flexible */}
            <div className="relative w-[90%] max-w-[616px] bg-white rounded-[16px] flex flex-col items-center justify-center p-8 md:p-12 shadow-xl animate-fade-in-up">

                {/* Close Button - Positioned top right */}
                <button
                    onClick={onClose}
                    className="absolute top-4 right-4 md:top-6 md:right-6 text-[#191919] hover:text-gray-600 transition-colors"
                >
                    <X size={24} />
                </button>

                {/* Logo Section */}
                <div className="flex flex-col items-center mb-10 md:mb-12 mt-4 md:mt-2">
                    {/* Authentic Figma Logo (Icon + Text) */}
                    <div className="w-[120px] h-[134px] md:w-[160px] md:h-[178px] flex items-center justify-center mb-4">
                ...
                <div className="w-full flex justify-center gap-[20px] md:gap-[40px] mb-8">
                    {/* Register Button - Solid Blue (#009bcd) */}
                    <button
                        onClick={onRegister}
                        className="px-[24px] md:px-[32px] py-[8px] rounded-[50px] bg-[#009bcd] text-white font-bold text-[14px] md:text-[16px] leading-[1.4] hover:bg-[#0089b6] transition-colors min-w-[100px] md:min-w-[120px]"
                    >
                        註冊
                    </button>
                    ...
                    <button
                        onClick={onLogin}
                        className="px-[24px] md:px-[32px] py-[8px] rounded-[50px] bg-gradient-to-r from-[#F2994A] to-[#009bcd] text-white font-bold text-[14px] md:text-[16px] leading-[1.4] hover:opacity-90 transition-opacity min-w-[100px] md:min-w-[120px]"
                    >
                        登入
                    </button>
                </div>
                ...
                <div className="w-full flex justify-center mb-8">
                    <button
                        onClick={onGuest}
                        className="text-[#666666] text-[14px] hover:text-[#191919] transition-colors underline decoration-1 underline-offset-4"
                    >
                        訪客瀏覽
                    </button>
                </div>
                ...
                <div className="w-full text-center mt-2">
                    <p className="text-[12px] md:text-[14px] text-[#191919] font-normal leading-[1.5]">
                        點擊註冊，即表示你同意
